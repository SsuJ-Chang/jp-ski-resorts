...
| 欄位 | 必填 | 型別 | 說明 |
|---|---:|---|---|
| `name` | 是 | string | 票種名稱，例如 `1 日券`、`4 小時券`、`夜滑券`。 |
| `priceLines` | 是 | string[] | 票價條列，一行代表一種可直接閱讀的價格單位。 |
| `note` | 否 | string | 只放會影響購票判斷的例外、限制或適用條件。 |

票種整理規則：

- 一個 `plan` 只代表一種票種，不要把多種方案硬塞進同一個標題。
- `priceLines` 以一行一個閱讀單位為原則，例如 `成人：¥7,000`、`小學生：¥3,000`。
- 複雜票價可依時段或購買管道拆成多行，例如 `平日：成人 ¥2,550、兒童 ¥2,040`。
- 票種名稱與票價文字盡量使用台灣繁體中文，必要時才保留日文 proper noun。
- 年齡分類統一使用 `成人`、`兒童`、`小學生`、`國中生`、`高中生`、`長者`；`Senior` 與 `シニア` 一律翻譯為 `長者`，不要使用 `樂齡` 或 `年長者` 作為票價分類。
- 使用半形 `/` 與 `~`，且前後保留空格，例如 `成人 / 長者`、`¥5,200 ~ ¥6,800`。
- `note` 只保留會影響決策的資訊，例如有效時間、連續使用限制、觀光限定、票種適用日期。
- 像苗場這種同時有共通券與區域券的雪場，應拆成不同 `plan`，不要混成同一張票。
...
| `publicTransit` | 否 | array | 不綁定主要城市的公共運輸或當地轉乘交通，例如最近車站、巴士站至雪場。 |
| `car` | 否 | object | 自駕資訊。 |
| `rjOpinion` | 否 | string | 顯示在交通方式區塊最下方的 RJ 個人心得。 |

### `access` 路線

`fromTokyo`、`fromOsaka`、`fromNagoya`、`fromSapporo`、`fromAirport` 與 `publicTransit` 都表示非自駕交通。前五者依出發地分類；`publicTransit` 用於不適合歸入主要城市或機場的當地公共運輸、接駁與轉乘路線。同一路線不要同時放在出發地欄位與 `publicTransit`。

自駕路線、交流道、開車時間、停車場、雪胎與雪鏈資訊一律放在 `access.car`，不要放進 `from*` 或 `publicTransit`。
...
可選的單一外部深度指南入口。只有在外部網站對該雪場提供具體且具差異性的專題內容時才使用；不要只用雪場名稱替換共用文案。

| 欄位 | 必填 | 類型 | 說明 |
|---|---:|---|---|
| `title` | 是 | string | 該雪場專屬的導流標題。 |
| `url` | 是 | URL | 外部深度指南網址。 |
