---
id: white-pia-takasu
name:
  zhTw: White Pia たかす
  ja: ホワイトピアたかす
  en: White Pia Takasu
region: chubu
prefecture: 岐阜縣
skiArea: takasu-mountains
tags:
- beginner_friendly
- family_friendly
- no_car_accessible
- snowboard_friendly
- onsen
links:
  official: https://whitepia.jp/
  trailMapPage: https://whitepia.jp/gerende/
  ticket: https://www.takasumountains.com/lift/
  access: https://whitepia.jp/access/
  snowReport: https://whitepia.jp/operation/
  liftStatus: https://whitepia.jp/operation/
  googleMaps: https://www.google.com/maps/search/?api=1&query=White%20Pia%20Takasu
contact:
  address:
    zhTw: 日本岐阜縣郡上市高鷲町鷲見 512
    ja: 〒501-5302 岐阜県郡上市高鷲町鷲見512
    googleMaps: https://www.google.com/maps/search/?api=1&query=White%20Pia%20Takasu
  phone: 0575-72-6311
season:
  label: 2025-2026
  operatingPeriod: 2025/11 中旬-2026/3 下旬，實際依積雪與官方公告調整
  hours: 全日 8:00-16:30；Sunrise 營業 7:00 起
  source: https://www.takasumountains.com/ski/
trailMaps:
- label: White Pia たかす雪場圖
  language: Japanese
  season: 2025-2026
  url: https://whitepia.jp/gerende/
  sourceLabel: White Pia Official
tickets:
  season: 2025-2026
  currency: JPY
  source: https://www.takasumountains.com/lift/
  note: TAKASU MOUNTAINS 6 雪場共通券摘要；單獨券與 Sunrise 營業請以官方頁面為準。
  plans:
  - name: 6 雪場共通 1 日券
    audience: 成人、兒童、長者、中高生
    price: 成人 ¥6,500、兒童 ¥3,800、長者 ¥5,500、中高生 ¥4,500
snowWeather:
  title: White Pia たかす雪況天氣
  provider: Official
  url: https://whitepia.jp/operation/
  snowDepth: 200 cm
  updatedAt: 2026/4/30 09:00:00
  forecast:
    - date: 12/27(六)
      weather: 雪
      low: "-5°C"
      high: "4°C"
    - date: 12/28(日)
      weather: 大雪
      low: "-8°C"
      high: "0°C"
    - date: 12/29(一)
      weather: 大雪
      low: "-9°C"
      high: "-2°C"
    - date: 12/30(二)
      weather: 陰
      low: "-5°C"
      high: "0°C"
    - date: 12/31(三)
      weather: 雪
      low: "-3°C"
      high: "3°C"
    - date: 1/1(四)
      weather: 晴
      low: "-4°C"
      high: "1°C"
    - date: 1/2(五)
      weather: 雨
      low: "8°C"
      high: "3°C"
  note: 雪況顯示沿用建站期間的靜態佔位資料；出發前請以官方即時雪況為準。
location:
  latitude: 35.949
  longitude: 136.879
elevation:
  verticalDrop: 355
courses:
  total: 13
  beginnerRatio: 30
  intermediateRatio: 40
  advancedRatio: 30
  courseInfoPage: https://whitepia.jp/gerende/
  summary: White Pia たかす官方標示 13 條 course、最長 2,600m，特色是熊手型 course layout、山頂展望、mogul、freestyle park 與鷲ヶ岳共通動線。
  details:
  - name: ブラックベア / Black Bear
    difficulty: beginner
    note: 下部為初級路線，Top season 中可接 Center Way，形成緩斜度動線；上部 Black Bear 則為上級路線。
  - name: センターウェイ / Center Way
    difficulty: beginner
    note: 官方說明為從 Black Bear 下部通往 base 的緩坡核心動線。
  - name: センターC / Center C
    difficulty: beginner
    note: 從 Alpha Quad / Delta Pair line 可滑入 Center Way 的初級選擇。
  - name: パノラマウェイ / Panorama Way
    difficulty: beginner
    note: 上部與下部皆列在官方 course 頁，尾根 course 可接往 Snake Street，初級者也能滑。
  - name: スネークストリート / Snake Street
    difficulty: beginner
    note: 從 Panorama Way 延伸的尾根線，官方說明初級者也可享受。
  - name: スターコース / Star Course
    difficulty: intermediate
    note: 位在 Echo Pair lift 後側，最後通過急斜面回到 lift 乘場前。
  - name: センターA / Center A
    difficulty: intermediate
    note: 官方將 Center A 標示為中級；mogul 初級與中級區也設於 Center A 上下部。
  - name: センターB / Center B
    difficulty: intermediate
    note: 從上部 line 接往 Center Way 的中級 course。
  - name: スイング / Swing
    difficulty: intermediate
    note: Bravo Quad line 的中級 course，接近 Panorama Way / Snake Street 一帶。
  - name: スキップ / Skip
    difficulty: intermediate
    note: 官方列為中級 course，位於 Bravo Quad line 的多分岐布局中。
  - name: ダイビング / Diving
    difficulty: advanced
    note: 官方列為上級 course，從上部 line 進入。
  - name: ダンシング / Dancing
    difficulty: advanced
    note: 官方列為上級 course，位於 Bravo Quad line 的急斜分岐。
  - name: ジャンピング / Jumping
    difficulty: advanced
    note: 官方列為上級 course，與 Dancing、Swing、Skip 等形成 White Pia 特有分岐。
lifts:
  total: 4
terrainSummary:
  beginner: Black Bear 下部、Center Way、Center C、Panorama Way 與 Snake Street 是主要緩坡動線。
  intermediate: Center A/B、Star、Swing、Skip 提供中級與 mogul 練習銜接。
  advanced: Diving、Dancing、Jumping 與 Black Bear 上部是主要上級線。
  snowboard: freestyle park、mogul 與多分岐地形讓 snowboarder 有不少玩法。
  powder: 官方重點較偏地形與展望，粉雪狀況需看當日公告。
sources:
- label: White Pia Gerende Information
  url: https://whitepia.jp/gerende/
- label: TAKASU MOUNTAINS Ski Area
  url: https://www.takasumountains.com/ski/
- label: TAKASU MOUNTAINS Lift Fee
  url: https://www.takasumountains.com/lift/
---
White Pia たかす是 Takasu Mountains 裡偏地形與展望特色的雪場，官方說明 course layout 呈熊手狀，從初級緩坡到 mogul、freestyle park 與高難度分岐都有。它也和鷲ヶ岳形成「ワシトピア」動線，適合安排同區滑行。
