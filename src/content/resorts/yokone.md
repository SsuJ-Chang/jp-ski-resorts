---
id: "yokone"
name:
  zhTw: "橫根"
  ja: "横根スキー場"
  en: "Yokone Ski Area"
region: "tohoku"
prefecture: "山形縣"
tags:
  - "night_skiing"
  - "no_car_accessible"
  - "onsen"
  - "snowboard_friendly"
visibility:
  status: "published"
  note: "山形縣雪場清單補新增的靜態初版；以官方資訊與公開雪場資料整理。"
links:
  official: "http://www.siroimori.co.jp/yokone-ski/index.html"
  weather: "https://weathernews.jp/ski/spot/33532/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=38.071059,139.730572"
  access: "http://www.siroimori.co.jp/yokone-ski/index.html"
contact:
  address:
    zhTw: "日本山形縣西置賜郡大字小国町大字小国小坂町895"
    ja: "山形県西置賜郡大字小国町大字小国小坂町895"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=38.071059,139.730572"
  phone: "0238-62-4723"
season:
  label: "2025-2026"
  operatingPeriod: "2025/12/下旬-2026/03/下旬"
  hours: "平日：9:00〜17:00 / 假日：9:00〜17:00 / 夜滑：17:00〜21:00(週日、祝休み)"
  note: "營業日期與時間會依積雪、天候與活動調整；出發前請確認官方公告。"
  source: "https://weathernews.jp/ski/spot/33532/"
snowWeather:
  title: "橫根 積雪與天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/33532/"
  snowDepth: "200 cm"
  updatedAt: "2026/4/30 09:00:00"
  forecast:
    - date: "12/27(六)"
      weather: "雪"
      low: "-5°C"
      high: "4°C"
    - date: "12/28(週日)"
      weather: "大雪"
      low: "-8°C"
      high: "0°C"
    - date: "12/29(一)"
      weather: "大雪"
      low: "-9°C"
      high: "-2°C"
    - date: "12/30(二)"
      weather: "陰"
      low: "-5°C"
      high: "0°C"
    - date: "12/31(三)"
      weather: "雪"
      low: "-3°C"
      high: "3°C"
    - date: "1/1(四)"
      weather: "晴"
      low: "-4°C"
      high: "1°C"
    - date: "1/2(五)"
      weather: "陰"
      low: "8°C"
      high: "3°C"
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 38.071059
  longitude: 139.730572
elevation:
  top: 279
  bottom: 140
  verticalDrop: 139
courses:
  total: 1
  beginnerRatio: 0
  intermediateRatio: 100
  advancedRatio: 0
  courseInfoPage: "https://weathernews.jp/ski/spot/33532/"
  summary: "橫根共有 1 條雪道，難度比例約初級 0%、中級 100%、進階 0%，適合依雪況與同行者程度安排滑行路線。"
lifts:
  total: 1
access:
  fromTokyo:
    - label: "大眾交通工具（主要車站銜接）"
      steps:
        - "鐵路：JR米坂線小国駅よりタクシー (5分)"
      estimatedTime: "約 5 分"
      difficulty: "medium"
  car:
    recommended: true
    snowTireRequired: true
    note: "停車場：186台 / 平日 免費 / 假日 免費。冬季山路需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 0%，可作為暖身、親子同行或初訪時的主要安排。"
  intermediate: "中級比例約 100%，適合已能穩定轉彎的滑雪者串接主要雪道。"
  advanced: "進階比例約 0%，建議視當日積雪與壓雪狀況選擇。"
  snowboard: "全面可；單板實際可滑區域仍以現場開放為準。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Yokone Ski Area Official Website"
    url: "http://www.siroimori.co.jp/yokone-ski/index.html"
  - label: "Weathernews 横根スキー場"
    url: "https://weathernews.jp/ski/spot/33532/"
---

橫根位於山形縣，白天與夜晚的雪面感不同，想延長滑行時間時會更有彈性。比起匆忙踩點，更適合用半天到一天熟悉雪面、坡感與山景；適合把運動量和放鬆感平衡在同一趟雪國旅程裡的人。整體適合放慢節奏，從雪面觸感、山景與停留氛圍中感受差異。
