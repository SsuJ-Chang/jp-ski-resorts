---
id: "f-village-snow-park"
name:
  zhTw: "F VILLAGE Snow Park"
  ja: "F VILLAGE Snow Park"
  en: "F VILLAGE Snow Park"
region: "hokkaido"
prefecture: "北海道"
tags:
  - "family_friendly"
  - "no_car_accessible"
  - "onsen"
visibility:
  status: "published"
  note: "北海道雪場清單補新增的靜態初版；以官方資訊與公開雪場資料整理。"
links:
  official: "https://shorefield.jp/fv-snowpark/"
  instagram: "https://www.instagram.com/fvsnowpark/"
  weather: "https://weathernews.jp/ski/spot/31417/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=42.990401,141.549772"
  access: "https://shorefield.jp/fv-snowpark/"
  ticket: "https://shorefield.jp/fv-snowpark/"
contact:
  address:
    zhTw: "日本北海道北広島市 Ｆビレッジ"
    ja: "〒061-1116 北海道北広島市 Ｆビレッジ"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=42.990401,141.549772"
season:
  label: "2025-2026"
  operatingPeriod: "2025/12/01-2026/02/末"
  hours: "10:00-16:00"
  note: "2025-2026 雪季官方公告於 2/28 結束營業；各活動區會依積雪與現場狀況調整開放。"
  source: "https://shorefield.jp/fv-snowpark/"
tickets:
  season: "2025-2026"
  currency: "JPY"
  source: "https://shorefield.jp/fv-snowpark/"
  note: "兒童票適用 12 歲以下；3 歲以下免費。滑雪／雪板套裝含專用票、雪具與雪衣租借、滑雪或雪板支援。"
  plans:
    - name: "Premium Pass"
      audience: "成人 / 兒童"
      price: "成人 ¥20,000、兒童 ¥17,000"
      note: "含滑雪／雪板與所有活動。"
    - name: "選べるセット"
      audience: "成人 / 兒童"
      price: "成人 ¥15,000、兒童 ¥12,000"
      note: "含滑雪／雪板與任選 2 種活動。"
    - name: "スキーorボードセット"
      audience: "成人 / 兒童"
      price: "成人 ¥11,000、兒童 ¥8,000"
      note: "滑雪或雪板基本套裝。"
    - name: "Premium Activity"
      audience: "4 歲以上"
      price: "¥11,000"
      note: "含所有活動與雪衣、雪靴、小物租借。"
    - name: "Activity 10"
      audience: "4 歲以上"
      price: "¥9,000"
      note: "含所有活動，不含雪衣與雪靴租借。"
    - name: "Activity 5"
      audience: "4 歲以上"
      price: "¥5,000"
      note: "可選 5 種活動，不含雪衣與雪靴租借。"
snowWeather:
  title: "F VILLAGE Snow Park 積雪與天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/31417/"
  snowDepth: 0 cm
  updatedAt: "2026/4/30 09:00:00"
  forecast: []
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 42.990401
  longitude: 141.549772
courses:
  total: 1
  beginnerRatio: 100
  intermediateRatio: 0
  advancedRatio: 0
  courseInfoPage: "https://shorefield.jp/fv-snowpark/"
  summary: "F VILLAGE Snow Park 以初次滑雪與雪板體驗區為主，設有 Snow Escalator 可上到雪丘上部反覆滑行；另有雪橇、tubing 與 Snow Mountain 等雪上活動區，屬於雪上體驗型園區。"
  details:
    - name: "SKI SNOWBOARD PARK"
      difficulty: beginner
      note: "滑雪／雪板體驗區內設有 Snow Escalator，可輕鬆移動到雪丘上部並反覆滑行；官方未公布一般雪道長度、斜度與分級。"
lifts:
  total: 1
access:
  fromSapporo:
    - label: "大眾交通工具（主要車站銜接）"
      steps:
        - "鐵路：札幌駅 (17分)"
        - "鐵路：新千歳空港 (20分)"
      estimatedTime: "約 17 分"
      difficulty: "medium"
  car:
    recommended: true
    snowTireRequired: true
    note: "自駕參考：自駕：札幌方面 (40分)；自駕：新千歳空港 (40分)。停車場：4000台。冬季山路需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "主要為初次滑雪與雪板體驗區，透過 Snow Escalator 上行後在短坡反覆練習。"
  intermediate: "未公布中級雪道；若已有基礎技術，較適合作為短時間暖身或親子同行體驗。"
  advanced: "未公布進階雪道；此處重點是雪上活動與初學體驗，而非長距離或高難度滑行。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "F VILLAGE Snow Park Official Website"
    url: "https://shorefield.jp/fv-snowpark/"
    note: "提供 2025-2026 營業資訊、活動區介紹、套裝票價、租借與交通資訊。"
  - label: "Weathernews F VILLAGE Snow Park"
    url: "https://weathernews.jp/ski/spot/31417/"
    note: "提供雪場天氣、營業期間、停車場、租借與基礎設施摘要。"
---

F VILLAGE Snow Park 位在北海道北廣島市的 F VILLAGE，主打初次滑雪、雪板與多種雪上活動。園區以短坡體驗和親子雪遊為核心，適合想用半天感受雪面、租借裝備並搭配溫泉與餐飲休息的人。
