---
id: "mashike-shokanbetsudake"
name:
  zhTw: "增毛町暑寒別岳"
  ja: "増毛町暑寒別岳"
  en: "Mashike Shokanbetsudake Ski Area"
region: "hokkaido"
prefecture: "北海道"
tags:
  - "beginner_friendly"
  - "night_skiing"
  - "onsen"
  - "snowboard_friendly"
visibility:
  status: "published"
  note: "北海道雪場清單補新增的靜態初版；以官方資訊與公開雪場資料整理。"
links:
  official: "https://www.town.mashike.hokkaido.jp/tourism/place/ski/index.html"
  weather: "https://weathernews.jp/ski/spot/31303/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=43.809878,141.529122"
  access: "https://www.town.mashike.hokkaido.jp/tourism/place/ski/index.html"
contact:
  address:
    zhTw: "日本北海道增毛郡增毛町暑寒澤698-1"
    ja: "〒077-0216 北海道増毛郡増毛町暑寒沢698-1"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=43.809878,141.529122"
  phone: "0164-53-3002"
season:
  label: "2025-2026"
  operatingPeriod: "2025/12/20-2026/03/08"
  hours: "平日：10:00〜17:00 / 假日：9:30〜17:00（日國定假日限定） / 夜滑：17:00〜21:00(1、2月の金・週六限定)"
  note: "營業日期與時間會依積雪、天候與活動調整；出發前請確認官方公告。"
  source: "https://weathernews.jp/ski/spot/31303/"
snowWeather:
  title: "增毛町暑寒別岳 積雪與天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/31303/"
  snowDepth: 0 cm
  updatedAt: "2026/4/30 09:00:00"
  forecast: []
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 43.809878
  longitude: 141.529122
elevation:
  top: 322
  bottom: 115
  verticalDrop: 207
courses:
  total: 7
  beginnerRatio: 50
  intermediateRatio: 30
  advancedRatio: 20
  courseInfoPage: "https://weathernews.jp/ski/spot/31303/"
  summary: "增毛町暑寒別岳共有 7 條雪道，難度比例約初級 50%、中級 30%、進階 20%，適合依雪況與同行者程度安排滑行路線。"
lifts:
  total: 2
access:
  car:
    recommended: true
    snowTireRequired: true
    note: "自駕參考：自駕：留萌IC (40分)。停車場：250台 / 平日 免費 / 假日 免費。冬季山路需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 50%，可作為暖身、親子同行或初訪時的主要安排。"
  intermediate: "中級比例約 30%，適合已能穩定轉彎的滑雪者串接主要雪道。"
  advanced: "進階比例約 20%，建議視當日積雪與壓雪狀況選擇。"
  snowboard: "全面可；單板實際可滑區域仍以現場開放為準。"
  powder: "バックカントリーコースを設置。未圧雪の林間を思いのままに滑走（※小学生以下は立入禁止）；粉雪與非壓雪區域會受天候與管理狀態影響。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Mashike Shokanbetsudake Ski Area Official Website"
    url: "https://www.town.mashike.hokkaido.jp/tourism/place/ski/index.html"
  - label: "Weathernews 増毛町暑寒別岳"
    url: "https://weathernews.jp/ski/spot/31303/"
---

增毛町暑寒別岳位於北海道，夜滑讓白天之外多一層雪場表情，短天數旅程也能拉出不同節奏。節奏不必排得太滿，反而能慢慢感受雪面、空氣與山景變化；適合想把滑雪後的泡湯、在地停留與雪國慢節奏放進同一天的人。滑行之外，也能把雪景、休息與地方氛圍一起留下來。
