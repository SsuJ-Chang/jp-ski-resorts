---
id: "hachimantai-tayama"
name:
  zhTw: "八幡平市田山"
  ja: "八幡平市田山スキー場"
  en: "Hachimantai City Tayama Ski Area"
region: "tohoku"
prefecture: "岩手縣"
tags:
  - "snowboard_friendly"
visibility:
  status: "published"
  note: "岩手縣雪場清單補新增的靜態初版；以官方資訊與公開雪場資料整理。"
links:
  official: "https://www.city.hachimantai.lg.jp/soshiki/shisho-a/2021.html"
  weather: "https://weathernews.jp/ski/spot/33303/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=Hachimantai%20City%20Tayama%20Ski%20Area"
contact:
  address:
    zhTw: "日本岩手縣八幡平市矢神 8-1"
    ja: "〒028-7603 岩手県八幡平市矢神8-1"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=Hachimantai%20City%20Tayama%20Ski%20Area"
  phone: "0195-73-2650"
season:
  label: "2025-2026"
  operatingPeriod: "2026/01/03-2026/03/22"
  hours: "9:00-16:00"
  note: "營業期間與時間為公開雪場資料；實際開放日、營業範圍與票價請以官方最新公告為準。"
  source: "https://weathernews.jp/ski/spot/33303/"
snowWeather:
  title: "八幡平市田山雪況天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/33303/"
  snowDepth: "200 cm"
  updatedAt: "2026/4/30 09:00:00"
  forecast:
    - date: "12/27(六)"
      weather: "雪"
      low: "-5°C"
      high: "4°C"
    - date: "12/28(週日)"
      weather: "大雪"
      low: "-8°C"
      high: "0°C"
    - date: "12/29(一)"
      weather: "大雪"
      low: "-9°C"
      high: "-2°C"
    - date: "12/30(二)"
      weather: "陰"
      low: "-5°C"
      high: "0°C"
    - date: "12/31(三)"
      weather: "雪"
      low: "-3°C"
      high: "3°C"
    - date: "1/1(四)"
      weather: "晴"
      low: "-4°C"
      high: "1°C"
    - date: "1/2(五)"
      weather: "陰"
      low: "8°C"
      high: "3°C"
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 40.134494
  longitude: 140.951911
elevation:
  top: 585
  bottom: 315
  verticalDrop: 270
courses:
  total: 2
  beginnerRatio: 20
  intermediateRatio: 40
  advancedRatio: 40
  courseInfoPage: "https://weathernews.jp/ski/spot/33303/"
  summary: "八幡平市田山共有 2 條雪道與 2 座纜車，標高約 315-585m；初級 20%、中級 40%、進階 40%，雪板可全區滑行。"
lifts:
  total: 2
access:
  fromTokyo:
    - label: "東京出發經田山"
      steps:
        - "從東京搭東北新幹線到盛岡站。"
        - "從盛岡站轉 JR 花輪線到田山站。"
        - "從田山站步行前往雪場。"
      estimatedTime: "田山站後約 15 分"
      difficulty: "medium"
      note: "也可搭盛岡－大館線高速巴士，於田山 Parking 下車後步行約 15 分；花輪線與高速巴士班次需先確認。"
  car:
    recommended: true
    snowTireRequired: true
    note: "自駕可由東北自動車道安代 IC 約 20 分抵達。停車場約 100 台，平日與假日免費；冬季需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 20%，初學者應先確認當日開放坡面。"
  intermediate: "中級比例約 40%，適合已有基礎轉彎能力的人。"
  advanced: "進階比例約 40%，小型雪場中進階比重較高。"
  snowboard: "雪板可全區滑行。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Hachimantai City Tayama Ski Area"
    url: "https://www.city.hachimantai.lg.jp/soshiki/shisho-a/2021.html"
  - label: "Weathernews 八幡平市田山スキー場"
    url: "https://weathernews.jp/ski/spot/33303/"
---

八幡平市田山位於岩手縣，不是追求壯觀規模，而是用安定坡面累積滑行手感與雪國記憶。比起匆忙踩點，更適合用半天到一天熟悉雪面、坡感與山景；適合同行者玩法不同時，依各自節奏安排滑行的人。整體適合放慢節奏，從雪面觸感、山景與停留氛圍中感受差異。
