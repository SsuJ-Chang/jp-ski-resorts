---
id: "kunimidaira"
name:
  zhTw: "國見平"
  ja: "国見平スキー場"
  en: "Kunimidaira Ski Area"
region: "tohoku"
prefecture: "岩手縣"
tags:
  - "snowboard_friendly"
visibility:
  status: "published"
  note: "岩手縣雪場清單補新增的靜態初版；以官方網站與公開雪場資料整理。"
links:
  official: "https://kunimidaira.com/"
  weather: "https://weathernews.jp/ski/spot/33314/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=Kunimidaira%20Ski%20Area"
contact:
  address:
    zhTw: "日本岩手縣奧州市衣川長塚 303"
    ja: "〒023-0000 岩手県奥州市衣川長塚303番地"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=Kunimidaira%20Ski%20Area"
  phone: "0197-52-6611"
season:
  label: "2025-2026"
  operatingPeriod: "2026/01/01-2026/03/16"
  hours: "8:30-16:30"
  note: "營業期間與時間為公開雪場資料；實際開放日、營業範圍與票價請以官方最新公告為準。"
  source: "https://weathernews.jp/ski/spot/33314/"
snowWeather:
  title: "國見平雪況天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/33314/"
  snowDepth: 0 cm
  updatedAt: "2026/4/30 09:00:00"
  forecast: []
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 39.060116
  longitude: 140.970125
elevation:
  top: 600
  bottom: 400
  verticalDrop: 200
courses:
  total: 11
  beginnerRatio: 30
  intermediateRatio: 40
  advancedRatio: 30
  courseInfoPage: "https://weathernews.jp/ski/spot/33314/"
  summary: "國見平共有 11 條雪道與 2 座纜車，標高約 400-600m；初級 30%、中級 40%、進階 30%，雪板可全區滑行。"
lifts:
  total: 2
access:
  car:
    recommended: true
    snowTireRequired: true
    note: "自駕可由平泉前澤 IC 或奧州 Smart IC 方向前往，約 30 分。停車場約 500 台，平日與假日免費；冬季需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 30%，可作為基礎練習與一般滑行的入門路線。"
  intermediate: "中級比例約 40%，是國見平主要滑行難度帶。"
  advanced: "進階比例約 30%，可提供一定坡度變化。"
  snowboard: "雪板可全區滑行。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Kunimidaira Ski Area Official Website"
    url: "https://kunimidaira.com/"
  - label: "Weathernews 国見平スキー場"
    url: "https://weathernews.jp/ski/spot/33314/"
---

國見平位於岩手縣，不是追求壯觀規模，而是用安定坡面累積滑行手感與雪國記憶。比起匆忙踩點，更適合用半天到一天熟悉雪面、坡感與山景；適合同行者玩法不同時，依各自節奏安排滑行的人。整體適合放慢節奏，從雪面觸感、山景與停留氛圍中感受差異。
