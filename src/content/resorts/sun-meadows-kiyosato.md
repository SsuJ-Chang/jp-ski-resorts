---
id: "sun-meadows-kiyosato"
name:
  zhTw: "Sun Meadows 清里"
  ja: "サンメドウズ清里 スキー場"
  en: "Sun Meadows Kiyosato Ski Area"
region: "kanto-koshinetsu"
prefecture: "山梨縣"
tags:
  - "beginner_friendly"
  - "family_friendly"
  - "snowboard_friendly"
links:
  official: "http://www.sunmeadows.co.jp/"
  instagram: "https://www.instagram.com/sunmeadows3302/"
  xTwitter: "https://twitter.com/sunmeadows"
  weather: "https://weathernews.jp/ski/spot/34903/"
  ticket: "https://weathernews.jp/ski/spot/34903/"
  access: "https://weathernews.jp/ski/spot/34903/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=35.947885,138.414259"
contact:
  address:
    zhTw: "日本山梨縣北杜市大泉町西井出 8240-1"
    ja: "〒407-0311 山梨県北杜市大泉町西井出8240-1"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=35.947885,138.414259"
  phone: "0551-48-4111"
season:
  label: "2025-2026"
  operatingPeriod: "2025/12/13-2026/4/5"
  hours: "平日 8:00-17:00；假日 8:00-17:00"
  note: "營業期間與時間可能依積雪、天候與整備狀況調整，出發前請確認官方公告。"
  source: "https://weathernews.jp/ski/spot/34903/"
tickets:
  season: "2025-2026"
  currency: "JPY"
  source: "https://weathernews.jp/ski/spot/34903/"
  note: "主要票種摘要；開幕優惠、季票與完整規則請以官方公告為準。"
  plans:
    - name: "1 日券"
      audience: "成人、兒童、銀髮族"
      price: "成人 ¥6,900、兒童 ¥5,500、銀髮族 ¥6,000"
    - name: "上午券"
      audience: "成人、兒童、銀髮族"
      price: "成人 ¥6,000、兒童 ¥4,600、銀髮族 ¥5,100"
    - name: "下午券"
      audience: "成人、兒童、銀髮族"
      price: "成人 ¥5,600、兒童 ¥4,600、銀髮族 ¥4,800"
    - name: "1 回券"
      audience: "成人"
      price: "成人 ¥1,000"
snowWeather:
  title: "Sun Meadows 清里雪況天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/34903/"
  snowDepth: 0 cm
  updatedAt: "2026/4/30 09:00:00"
  forecast: []
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 35.947885
  longitude: 138.414259
elevation:
  top: 1900
  bottom: 1600
  verticalDrop: 300
courses:
  total: 8
  beginnerRatio: 40
  intermediateRatio: 45
  advancedRatio: 15
  courseInfoPage: "https://weathernews.jp/ski/spot/34903/"
  summary: "清里高原一帶的雪場，共有 8 條雪道，初級與中級比例高，山頂標高約 1,900m。"
  details:
    - name: "初級路線 1"
      difficulty: beginner
      note: "目前僅能確認此雪場包含初級滑行比例；正式雪道名稱、長度與坡度待後續以官方雪道圖補齊。"
    - name: "初級路線 2"
      difficulty: beginner
      note: "目前僅能確認此雪場包含初級滑行比例；正式雪道名稱、長度與坡度待後續以官方雪道圖補齊。"
    - name: "初級路線 3"
      difficulty: beginner
      note: "目前僅能確認此雪場包含初級滑行比例；正式雪道名稱、長度與坡度待後續以官方雪道圖補齊。"
    - name: "中級路線 1"
      difficulty: intermediate
      note: "目前僅能確認此雪場包含中級滑行比例；正式雪道名稱、長度與坡度待後續以官方雪道圖補齊。"
    - name: "中級路線 2"
      difficulty: intermediate
      note: "目前僅能確認此雪場包含中級滑行比例；正式雪道名稱、長度與坡度待後續以官方雪道圖補齊。"
    - name: "中級路線 3"
      difficulty: intermediate
      note: "目前僅能確認此雪場包含中級滑行比例；正式雪道名稱、長度與坡度待後續以官方雪道圖補齊。"
    - name: "中級路線 4"
      difficulty: intermediate
      note: "目前僅能確認此雪場包含中級滑行比例；正式雪道名稱、長度與坡度待後續以官方雪道圖補齊。"
    - name: "上級路線"
      difficulty: advanced
      note: "目前僅能確認此雪場包含上級滑行比例；正式雪道名稱、長度與坡度待後續以官方雪道圖補齊。"
lifts:
  total: 3
access:
  fromTokyo:
    - label: "清里站轉計程車"
      steps:
        - "從東京方向搭 JR 中央本線與小海線到清里站。"
        - "從清里站轉計程車前往雪場。"
      estimatedTime: "約 5 分鐘"
      difficulty: "medium"
      note: "時間為清里站到雪場的計程車段；鐵路銜接與計程車需事先確認。"
  car:
    recommended: true
    snowTireRequired: true
    note: "從中央道長坂 IC 經一般道路前往約 90 分鐘；停車場約 1,200 台，平日與假日免費。"
terrainSummary:
  beginner: "初級雪道約 40%，適合清里住宿或親子行程銜接。"
  intermediate: "中級雪道約 45%，是主要滑行比例。"
  advanced: "高級雪道約 15%，可作為進階練習。"
  snowboard: "雪板可全區滑行。"
sources:
  - label: "Sun Meadows Kiyosato Official"
    url: "http://www.sunmeadows.co.jp/"
  - label: "Weathernews サンメドウズ清里 スキー場"
    url: "https://weathernews.jp/ski/spot/34903/"
---

Sun Meadows 清里位於山梨縣，高海拔帶來清爽雪質與開闊視野，山岳景觀會成為滑行之外的亮點。節奏不必排得太滿，反而能慢慢感受雪面、空氣與山景變化；適合親子同行或想用輕鬆節奏累積滑行經驗的人。滑行之外，也能把雪景、休息與地方氛圍一起留下來。
