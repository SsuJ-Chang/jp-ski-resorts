---
id: "mount-racey"
name:
  zhTw: "Mount Racey"
  ja: "マウントレースイスキーリゾート"
  en: "Mount Racey Ski Resort"
region: "hokkaido"
prefecture: "北海道"
tags:
  - "family_friendly"
  - "large_ski_area"
  - "no_car_accessible"
  - "onsen"
  - "powder"
  - "snowboard_friendly"
visibility:
  status: "published"
  note: "北海道雪場清單補新增的靜態初版；以官方資訊與公開雪場資料整理。"
links:
  official: "https://yubari-resort.co.jp/"
  facebook: "https://www.facebook.com/mountracey.skiresort"
  instagram: "https://www.instagram.com/mt.racey_ski/"
  weather: "https://weathernews.jp/ski/spot/31608/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=43.050830,141.969730"
  access: "https://yubari-resort.co.jp/"
contact:
  address:
    zhTw: "日本北海道夕張市末広2-4"
    ja: "〒068-0411 北海道夕張市末広2-4"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=43.050830,141.969730"
  phone: "0123-57-7766"
season:
  label: "2025-2026"
  operatingPeriod: "2025/12/06-2026/03/29"
  hours: "平日：9:00〜16:00 / 假日：9:00〜16:00 / 夜滑：無夜滑營業"
  note: "營業日期與時間會依積雪、天候與活動調整；出發前請確認官方公告。"
  source: "https://weathernews.jp/ski/spot/31608/"
snowWeather:
  title: "Mount Racey 積雪與天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/31608/"
  snowDepth: 0 cm
  updatedAt: "2026/4/30 09:00:00"
  forecast: []
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 43.05083
  longitude: 141.96973
elevation:
  top: 702
  bottom: 298
  verticalDrop: 404
courses:
  total: 18
  beginnerRatio: 20
  intermediateRatio: 40
  advancedRatio: 40
  courseInfoPage: "https://weathernews.jp/ski/spot/31608/"
  summary: "Mount Racey共有 18 條雪道，難度比例約初級 20%、中級 40%、進階 40%，適合依雪況與同行者程度安排滑行路線。"
lifts:
  total: 3
access:
  fromSapporo:
    - label: "大眾交通工具（主要車站銜接）"
      steps:
        - "鐵路：新千歳空港（快速エアポート）→南千歳（JR石勝線）→新夕張駅 ※新夕張駅前→レースイリゾート前（夕鉄バス利用） (90分)"
        - "鐵路：札幌→新夕張駅 ※新夕張駅前→レースイリゾート前（夕鉄バス利用） (110分)"
      estimatedTime: "約 90 分"
      difficulty: "medium"
  fromAirport:
    - label: "機場銜接"
      steps:
        - "飛機：免費シャトル送迎は暫停營運となります。"
      difficulty: "medium"
  car:
    recommended: true
    snowTireRequired: true
    note: "自駕參考：自駕：札幌から国道337号線→国道274号線三川交差点を左折し、国道234号線→栗山町角田交差点より道道3号線→夕張へ (82分)；自駕：道東道 夕張IC (25分)。停車場：1500台 / 平日 免費 / 假日 免費。冬季山路需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 20%，可作為暖身、親子同行或初訪時的主要安排。"
  intermediate: "中級比例約 40%，適合已能穩定轉彎的滑雪者串接主要雪道。"
  advanced: "進階比例約 40%，建議視當日積雪與壓雪狀況選擇。"
  snowboard: "全面可；單板實際可滑區域仍以現場開放為準。"
  powder: "○ / カービングライン、最大43度の急斜面が未圧雪コースで、新雪が降ったら、最高のパウダーエリアになります。；粉雪與非壓雪區域會受天候與管理狀態影響。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Mount Racey Ski Resort Official Website"
    url: "https://yubari-resort.co.jp/"
  - label: "Weathernews マウントレースイスキーリゾート"
    url: "https://weathernews.jp/ski/spot/31608/"
---

Mount Racey位於北海道，粉雪與非壓雪地形讓滑行更有探索感，適合想離開單調壓雪面的人。降雪後的雪面變化會讓每一趟滑行有不同表情；適合想把滑雪後的泡湯、在地停留與雪國慢節奏放進同一天的人。滑行之外，也能把雪景、休息與地方氛圍一起留下來。
