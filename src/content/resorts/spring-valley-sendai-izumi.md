---
id: "spring-valley-sendai-izumi"
name:
  zhTw: "Spring Valley 仙台泉"
  ja: "スプリングバレー仙台泉スキー場"
  en: "Spring Valley Sendai Izumi Ski Area"
region: "tohoku"
prefecture: "宮城縣"
tags:
  - "family_friendly"
  - "large_ski_area"
  - "night_skiing"
  - "no_car_accessible"
  - "snowboard_friendly"
visibility:
  status: "published"
  note: "宮城縣雪場清單補新增的靜態初版；以官方資訊與公開雪場資料整理。"
links:
  official: "https://www.springvalley.co.jp/"
  weather: "https://weathernews.jp/ski/spot/33410/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=38.418528,140.722868"
  access: "https://www.springvalley.co.jp/"
contact:
  address:
    zhTw: "日本宮城縣仙台市泉区福岡字岳山14-2"
    ja: "〒981-3225 宮城県仙台市泉区福岡字岳山14-2"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=38.418528,140.722868"
  phone: "022-379-3755"
season:
  label: "2025-2026"
  operatingPeriod: "2025/12/17-2026/04/01"
  hours: "平日：9:00〜17:00 / 假日：9:00〜17:00 / 夜滑：17:00〜22:00※週五、週六は〜23時00分"
  note: "營業日期與時間會依積雪、天候與活動調整；出發前請確認官方公告。"
  source: "https://weathernews.jp/ski/spot/33410/"
snowWeather:
  title: "Spring Valley 仙台泉 積雪與天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/33410/"
  snowDepth: "200 cm"
  updatedAt: "2026/4/30 09:00:00"
  forecast:
    - date: "12/27(六)"
      weather: "雪"
      low: "-5°C"
      high: "4°C"
    - date: "12/28(週日)"
      weather: "大雪"
      low: "-8°C"
      high: "0°C"
    - date: "12/29(一)"
      weather: "大雪"
      low: "-9°C"
      high: "-2°C"
    - date: "12/30(二)"
      weather: "陰"
      low: "-5°C"
      high: "0°C"
    - date: "12/31(三)"
      weather: "雪"
      low: "-3°C"
      high: "3°C"
    - date: "1/1(四)"
      weather: "晴"
      low: "-4°C"
      high: "1°C"
    - date: "1/2(五)"
      weather: "陰"
      low: "8°C"
      high: "3°C"
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 38.418528
  longitude: 140.722868
elevation:
  top: 959
  bottom: 687
  verticalDrop: 272
courses:
  total: 12
  beginnerRatio: 25
  intermediateRatio: 50
  advancedRatio: 25
  courseInfoPage: "https://weathernews.jp/ski/spot/33410/"
  summary: "Spring Valley 仙台泉共有 12 條雪道，難度比例約初級 25%、中級 50%、進階 25%，適合依雪況與同行者程度安排滑行路線。"
lifts:
  total: 4
access:
  fromTokyo:
    - label: "大眾交通工具（主要車站銜接）"
      steps:
        - "鐵路：仙台市営地下鉄泉中央駅下車、バスで (50分)"
      estimatedTime: "約 50 分"
      difficulty: "medium"
  car:
    recommended: true
    snowTireRequired: true
    note: "自駕參考：自駕：東北自動車道ETC専用泉PAスマートIC (35分)；自駕：東北自動車道仙台宮城ICまたは泉IC (50分)。停車場：600台 / 平日 免費 / 假日 500円（12/29〜1/3含む）。冬季山路需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 25%，可作為暖身、親子同行或初訪時的主要安排。"
  intermediate: "中級比例約 50%，適合已能穩定轉彎的滑雪者串接主要雪道。"
  advanced: "進階比例約 25%，建議視當日積雪與壓雪狀況選擇。"
  snowboard: "全面可；單板實際可滑區域仍以現場開放為準。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Spring Valley Sendai Izumi Ski Area Official Website"
    url: "https://www.springvalley.co.jp/"
  - label: "Weathernews スプリングバレー仙台泉スキー場"
    url: "https://weathernews.jp/ski/spot/33410/"
---

Spring Valley 仙台泉位於宮城縣，白天與夜晚的雪面感不同，想延長滑行時間時會更有彈性。比起匆忙踩點，更適合用半天到一天熟悉雪面、坡感與山景；適合把下班後、午後或夜間練習納入安排的人。整體適合放慢節奏，從雪面觸感、山景與停留氛圍中感受差異。
