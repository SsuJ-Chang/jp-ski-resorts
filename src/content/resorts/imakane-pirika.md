---
id: "imakane-pirika"
name:
  zhTw: "今金町 Pirika"
  ja: "今金町ピリカスキー場"
  en: "Imakane Pirika Ski Area"
region: "hokkaido"
prefecture: "北海道"
tags:
  - "family_friendly"
  - "night_skiing"
  - "no_car_accessible"
  - "onsen"
  - "snowboard_friendly"
visibility:
  status: "published"
  note: "北海道雪場清單補新增的靜態初版；以官方資訊與公開雪場資料整理。"
links:
  official: "https://kurplazapirika.net"
  weather: "https://weathernews.jp/ski/spot/32402/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=42.464814,140.203654"
  access: "https://kurplazapirika.net"
contact:
  address:
    zhTw: "日本北海道瀬棚郡今金町字美利河205-1"
    ja: "〒049-4151 北海道瀬棚郡今金町字美利河205-1"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=42.464814,140.203654"
  phone: "0137-83-7111"
season:
  label: "2025-2026"
  operatingPeriod: "2025/12/20-2026/03/22"
  hours: "平日：9:00〜16:00 / 假日：9:00〜16:00 / 夜滑：16:00〜21:00"
  note: "營業日期與時間會依積雪、天候與活動調整；出發前請確認官方公告。"
  source: "https://weathernews.jp/ski/spot/32402/"
snowWeather:
  title: "今金町 Pirika 積雪與天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/32402/"
  snowDepth: "200 cm"
  updatedAt: "2026/4/30 09:00:00"
  forecast:
    - date: "12/27(六)"
      weather: "雪"
      low: "-5°C"
      high: "4°C"
    - date: "12/28(週日)"
      weather: "大雪"
      low: "-8°C"
      high: "0°C"
    - date: "12/29(一)"
      weather: "大雪"
      low: "-9°C"
      high: "-2°C"
    - date: "12/30(二)"
      weather: "陰"
      low: "-5°C"
      high: "0°C"
    - date: "12/31(三)"
      weather: "雪"
      low: "-3°C"
      high: "3°C"
    - date: "1/1(四)"
      weather: "晴"
      low: "-4°C"
      high: "1°C"
    - date: "1/2(五)"
      weather: "陰"
      low: "8°C"
      high: "3°C"
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 42.464814
  longitude: 140.203654
elevation:
  top: 410
  bottom: 133
  verticalDrop: 277
courses:
  total: 5
  beginnerRatio: 20
  intermediateRatio: 50
  advancedRatio: 30
  courseInfoPage: "https://weathernews.jp/ski/spot/32402/"
  summary: "今金町 Pirika共有 5 條雪道，難度比例約初級 20%、中級 50%、進階 30%，適合依雪況與同行者程度安排滑行路線。"
lifts:
  total: 1
access:
  fromSapporo:
    - label: "大眾交通工具（主要車站銜接）"
      steps:
        - "鐵路：函館本線長万部駅下車、バスで (30分)"
      estimatedTime: "約 30 分"
      difficulty: "medium"
  car:
    recommended: true
    snowTireRequired: true
    note: "自駕參考：自駕：長万部から約30キロ (25分)；自駕：国縫ICから約15キロ (15分)。停車場：80台 / 平日 免費 / 假日 免費。冬季山路需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 20%，可作為暖身、親子同行或初訪時的主要安排。"
  intermediate: "中級比例約 50%，適合已能穩定轉彎的滑雪者串接主要雪道。"
  advanced: "進階比例約 30%，建議視當日積雪與壓雪狀況選擇。"
  snowboard: "全面可；單板實際可滑區域仍以現場開放為準。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Imakane Pirika Ski Area Official Website"
    url: "https://kurplazapirika.net"
  - label: "Weathernews 今金町ピリカスキー場"
    url: "https://weathernews.jp/ski/spot/32402/"
---

今金町 Pirika位於北海道，夜滑讓白天之外多一層雪場表情，短天數旅程也能拉出不同節奏。節奏不必排得太滿，反而能慢慢感受雪面、空氣與山景變化；適合想把滑雪後的泡湯、在地停留與雪國慢節奏放進同一天的人。
