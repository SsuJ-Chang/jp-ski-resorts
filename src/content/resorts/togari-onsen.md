---
id: togari-onsen
name:
  zhTw: 戶狩溫泉
  ja: 戸狩温泉スキー場
  en: TOGARI SKI RESORT
region: kanto-koshinetsu
prefecture: 長野縣
tags:
- family_friendly
- night_skiing
- no_car_accessible
- onsen
- powder
- snowboard_friendly
links:
  official: https://togari.jp
  weather: https://weathernews.jp/ski/spot/34832/
  access: https://togari.jp/winter/access/
  googleMaps: https://www.google.com/maps/search/?api=1&query=Togari%20Onsen%20Ski%20Resort
contact:
  address:
    zhTw: 長野縣飯山市大字豐田 6356-2
    ja: 〒389-2411 長野県飯山市大字豊田6356-2
    googleMaps: https://www.google.com/maps/search/?api=1&query=Togari%20Onsen%20Ski%20Resort
  phone: 0269-65-2359
season:
  label: 2025-2026
  operatingPeriod: 2025/12/20-2026/3/29
  hours: 平日 8:30-16:30、假日 8:30-16:30
  nightSkiingHours: 有夜滑，日期與區域請以官方公告為準
  note: 有夜滑；實際日期、區域與活動請以官方公告為準。
  source: https://weathernews.jp/ski/spot/34832/
tickets:
  season: 2025-2026
  currency: JPY
  source: https://weathernews.jp/ski/spot/34832/
  note: 主要票種依 2025-2026 公開雪場資料整理；完整票價、夜滑與活動票種請以官方為準。
  plans:
  - name: 1 日券
    audience: 成人、兒童、長者
    price: 成人 ¥5,500、兒童 ¥3,500、長者 ¥5,000
  - name: 2 日券
    audience: 成人、兒童、長者
    price: 成人 ¥9,500、兒童 ¥5,500、長者 ¥9,000
  - name: 上午券
    audience: 成人、兒童、長者
    price: 成人 ¥4,500、兒童 ¥3,000、長者 ¥4,000
    note: 8:30-13:00。
  - name: 下午券
    audience: 成人、兒童、長者
    price: 成人 ¥4,500、兒童 ¥3,000、長者 ¥4,000
    note: 12:30-17:00。
snowWeather:
  title: 戶狩溫泉雪況天氣
  provider: Weathernews
  url: https://weathernews.jp/ski/spot/34832/
  snowDepth: 200 cm
  updatedAt: 2026/4/30 09:00:00
  forecast:
    - date: 12/27(六)
      weather: 雪
      low: "-5°C"
      high: "4°C"
    - date: 12/28(週日)
      weather: 大雪
      low: "-8°C"
      high: "0°C"
    - date: 12/29(一)
      weather: 大雪
      low: "-9°C"
      high: "-2°C"
    - date: 12/30(二)
      weather: 陰
      low: "-5°C"
      high: "0°C"
    - date: 12/31(三)
      weather: 雪
      low: "-3°C"
      high: "3°C"
    - date: 1/1(四)
      weather: 晴
      low: "-4°C"
      high: "1°C"
    - date: 1/2(五)
      weather: 雨
      low: "8°C"
      high: "3°C"
  note: 雪況與天氣會隨季節、營運狀況與天候快速變動，出發前請以雪場當季公告與最新天氣資訊為準。
location:
  latitude: 36.932193
  longitude: 138.387866
elevation:
  top: 1050
  bottom: 400
  verticalDrop: 650
courses:
  total: 17
  beginnerRatio: 40
  intermediateRatio: 40
  advancedRatio: 20
  courseInfoPage: https://togari.jp/winter/gelande/
  summary: 官方 field 頁列出アースドラゴン與スカイドラゴン兩大 field 的 course 狀況與介紹，包含 Dragon Park、連絡路、powder、mogul 與 panoramic course。
  details:
  - name: スパイクテール
    difficulty: beginner
    note: 設於 course 旁、可輕鬆嘗試的 mogul beginner route。
  - name: テールライン
    difficulty: beginner
    length: 1,080m
    note: 戶狩溫泉內的長距離路線，官方介紹為寬廣緩斜面，適合滑雪與 snowboard debut。
  - name: コンタクトラインB
    difficulty: beginner
    note: 初學者也能前往アーストップリフト的緩斜連絡路。
  - name: スケールライン
    difficulty: beginner
    note: 初級者可繞行並安心接往テールライン。
  - name: ウィスカーライン
    difficulty: beginner
    length: 420m
    note: 連接クラウドライン下部的初學者迂迴路線，可前往スカイドラゴン各リフト。
  - name: ブレスライン
    difficulty: beginner
    length: 250m
    note: 與上級者路線並行、景觀好的初學者路線。
  - name: ファングライン
    difficulty: beginner
    length: 730m
    note: 從山頂欣賞景色後以緩斜面接往アースドラゴンフィールド下部。
  - name: コンタクトラインA
    difficulty: beginner
    note: 由スカイドラゴン下部回到アーステールリフト與基地側的連絡路。
  - name: スパイクライン
    difficulty: intermediate
    length: 540m
    maxSlope: 21°
    averageSlope: 18°
    note: 全長超過 300m 的 SAJ 公認 mogul course，官方介紹為大規模技術斜面。
  - name: フットライン
    difficulty: intermediate
    note: 週末中午會再壓雪整備的 special long burn。
  - name: バックボーン
    difficulty: intermediate
    length: 1,370m
    maxSlope: 14°
    averageSlope: 11°
    note: 官方標示為雪場最長路線，坡度相對平緩並穿越林間景色。
  - name: クラウドライン
    difficulty: intermediate
    length: 800m
    maxSlope: 24°
    averageSlope: 20°
    note: 視野開闊的 panoramic 中級者路線。
  - name: アイズライン
    difficulty: intermediate
    length: 250m
    maxSlope: 25°
    averageSlope: 19°
    note: 適合 ski 與 snowboard carving turn 的中級者路線。
  - name: ドラゴンパーク
    difficulty: advanced
    note: Jump 與 jib 等 freestyle course，官方介紹可依程度自由挑戰。
  - name: ウイングライン
    difficulty: advanced
    length: 750m
    maxSlope: 29°
    averageSlope: 17°
    note: 可 powder surfing 的寬廣非壓雪 DEEP POWDER 路線。
  - name: クローライン
    difficulty: advanced
    length: 320m
    maxSlope: 34°
    averageSlope: 24°
    note: アースドラゴンフィールド最大急斜面，可高速 powder run。
  - name: ドラゴンヘッド
    difficulty: advanced
    length: 250m
    maxSlope: 25°
    averageSlope: 23°
    note: 具獨特地形與粉雪的路線，山頂景觀佳且有初級路線並行。
lifts:
  total: 4
access:
  fromTokyo:
    - label: "東京出發經飯山"
      steps:
        - "從東京搭北陸新幹線到飯山站。"
        - "從飯山站搭直結接駁巴士或計程車前往戶狩溫泉。"
      estimatedTime: "飯山後約 20 分"
      difficulty: "easy"
      note: "滑雪季有飯山站與雪場之間的接駁巴士，也可從戶狩野澤溫泉站搭計程車約 5 分；班次、費用與停靠點需先確認。"
  car:
    recommended: true
    snowTireRequired: true
    note: "從上信越自動車道豐田飯山 IC 約 25 分。普通車停車免費，巴士車輛另收費；停車場不建議車宿，冬季需準備雪胎或雪鏈。"
terrainSummary:
  beginner: 初級約 40%，可從山麓與主要區域安排練習。
  intermediate: 中級約 40%，適合有基礎後銜接不同區域。
  advanced: 上級約 20%，公開資料提到地形、park 與 mogul 等玩法。
  powder: 戶狩溫泉可期待粉雪與地形變化，實際仍需看當日開放。
sources:
- label: TOGARI SKI RESORT Official Website
  url: https://togari.jp
- label: Weathernews 戸狩温泉スキー場
  url: https://weathernews.jp/ski/spot/34832/
- label: Weathernews 中部人氣排名
  url: https://weathernews.jp/ski/area/chubu/ranking.html
- label: TOGARI SKI RESORT Access
  url: https://togari.jp/winter/access/
---

戶狩溫泉位於長野縣，想在一般滑行之外加入小跳、道具與節奏變化，這裡會更有玩味。降雪後的雪面變化會讓每一趟滑行有不同表情；適合把運動量和放鬆感平衡在同一趟雪國旅程裡的人。整體適合放慢節奏，從雪面觸感、山景與停留氛圍中感受差異。
