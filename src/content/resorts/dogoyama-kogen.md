---
id: "dogoyama-kogen"
name:
  zhTw: "道後山高原"
  ja: "道後山高原スキー場"
  en: "Dogoyama Kogen Ski Area"
region: "kansai-chugoku"
prefecture: "廣島縣"
tags:
  - "family_friendly"
  - "no_car_accessible"
  - "onsen"
  - "snowboard_friendly"
links:
  official: "https://www.dogoyamakogen.com/"
  trailMapPage: "https://www.dogoyamakogen.com/"
  weather: "https://weathernews.jp/ski/spot/36701/"
  googleMaps: "https://maps.google.co.jp/maps?q=35.056415,133.203524"
  access: "https://www.dogoyamakogen.com/"
contact:
  address:
    zhTw: "廣島縣庄原市西城町三坂金屋子"
    ja: "〒729-5611 広島県庄原市西城町三坂金屋子"
    googleMaps: "https://maps.google.co.jp/maps?q=35.056415,133.203524"
  phone: "0824-84-2105"
season:
  label: "2025-2026"
  operatingPeriod: "2026/01/04-2026/02/22"
  hours: "平日：8:00〜17:00（纜車 8:30〜16:30） / 假日：8:00〜17:00（纜車 8:30〜16:30）"
  note: "營業日期與時間會依積雪、天候與活動調整；出發前請確認官方公告。"
  source: "https://weathernews.jp/ski/spot/36701/"
trailMaps:
  - label: "道後山高原官方雪場資訊"
    language: "日本語"
    season: "2025-2026"
    url: "https://www.dogoyamakogen.com/"
    sourceLabel: "Dogoyama Kogen Official"
tickets:
  season: "2025-2026"
  currency: "JPY"
  source: "https://weathernews.jp/ski/spot/36701/"
  note: "2025-2026 主要票種摘要；實際販售條件、日期與折扣請以官方公告為準。"
  plans:
    - name: "1 日券"
      price: "成人 ¥3,500、兒童 ¥2,000、樂齡 ¥2,000"
    - name: "6 回券"
      price: "成人 ¥2,000"
    - name: "上午券"
      price: "成人 ¥2,000"
    - name: "下午券"
      price: "成人 ¥2,000"
    - name: "1 回券"
      price: "成人 ¥500"
    - name: "女士日 1 日券"
      price: "成人 ¥1,000"
      note: "每週三女性限定。"
snowWeather:
  title: "道後山高原雪況天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/36701/"
  snowDepth: "200 cm"
  updatedAt: "2026/4/30 09:00:00"
  forecast:
    - date: "12/27(六)"
      weather: "雪"
      low: "-5°C"
      high: "4°C"
    - date: "12/28(週日)"
      weather: "大雪"
      low: "-8°C"
      high: "0°C"
    - date: "12/29(一)"
      weather: "大雪"
      low: "-9°C"
      high: "-2°C"
    - date: "12/30(二)"
      weather: "陰"
      low: "-5°C"
      high: "0°C"
    - date: "12/31(三)"
      weather: "雪"
      low: "-3°C"
      high: "3°C"
    - date: "1/1(四)"
      weather: "晴"
      low: "-4°C"
      high: "1°C"
    - date: "1/2(五)"
      weather: "陰"
      low: "8°C"
      high: "3°C"
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 35.056415
  longitude: 133.203524
elevation:
  top: 1050
  bottom: 700
  verticalDrop: 350
courses:
  total: 4
  beginnerRatio: 40
  intermediateRatio: 40
  advancedRatio: 20
  courseInfoPage: "https://www.dogoyamakogen.com/"
  summary: "道後山高原共有 4 條雪道，難度比例約初級 40%、中級 40%、進階 20%，是廣島縣北部可搭配溫泉停留的地方型雪場。"
lifts:
  total: 2
access:
  fromOsaka:
    - label: "大眾交通工具（主要車站銜接）"
      steps:
        - "搭乘 JR 藝備線至備後落合站，再轉乘計程車約 20 分鐘前往雪場。"
      estimatedTime: "約 20 分"
      difficulty: "medium"
      note: "車站後段以計程車銜接為主，冬季建議先確認叫車、回程與營業狀態。"
  car:
    recommended: true
    snowTireRequired: true
    note: "自駕參考：中國自動車道庄原 IC 經 R183 往米子方向約 40 分；或從東城 IC 經 R375、R183 往米子方向約 40 分。停車場約 800 台，平日與假日免費；冬季山路需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 40%，可安排為暖身、親子同行或初訪時的主要滑行區域。"
  intermediate: "中級比例約 40%，適合已能穩定轉彎的滑雪者串接主要雪道。"
  advanced: "進階比例約 20%，建議依當日積雪、壓雪與開放狀況選擇。"
  snowboard: "雪板可全區滑行；實際可滑區域仍以現場公告為準。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Dogoyama Kogen Official"
    url: "https://www.dogoyamakogen.com/"
  - label: "道後山高原 Weathernews"
    url: "https://weathernews.jp/ski/spot/36701/"
---

道後山高原位於廣島縣，重點不在刺激挑戰，而是讓同行者用舒服節奏熟悉雪地活動。行程可以安排得單純一點，把時間留給練習、休息與看雪景；適合不只想滑雪，也想把身體放鬆與地方氛圍一起安排的人。停留時間可依練習、休息與雪景體驗彈性安排，不必把一天排得太緊。
