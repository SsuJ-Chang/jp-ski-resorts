---
id: "dynasty-ski-resort"
name:
  zhTw: "Dynasty Ski Resort"
  ja: "ダイナスティスキーリゾート"
  en: "Dynasty Ski Resort"
region: "hokkaido"
prefecture: "北海道"
tags:
  - "beginner_friendly"
  - "family_friendly"
  - "night_skiing"
  - "no_car_accessible"
  - "onsen"
  - "snowboard_friendly"
visibility:
  status: "published"
  note: "北海道雪場清單補新增的靜態初版；以官方資訊與公開雪場資料整理。"
links:
  official: "http://www.dynasty-resort.jp/"
  weather: "https://weathernews.jp/ski/spot/31416/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=42.931520,141.451489"
  access: "http://www.dynasty-resort.jp/"
contact:
  address:
    zhTw: "日本北海道北広島市仁別82番4"
    ja: "〒061-1267 北海道北広島市仁別82番4"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=42.931520,141.451489"
  phone: "011-376-2611"
season:
  label: "2025-2026"
  operatingPeriod: "2025/12/28-2026/03/15"
  hours: "平日：9:00〜21:00 / 休日：9:00〜21:00 / ナイター：16:00〜21:00"
  note: "營業日期與時間會依積雪、天候與活動調整；出發前請確認官方公告。"
  source: "https://weathernews.jp/ski/spot/31416/"
snowWeather:
  title: "Dynasty Ski Resort 積雪與天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/31416/"
  snowDepth: "200 cm"
  updatedAt: "2026/4/30 09:00:00"
  forecast:
    - date: "12/27(六)"
      weather: "雪"
      low: "-5°C"
      high: "4°C"
    - date: "12/28(日)"
      weather: "大雪"
      low: "-8°C"
      high: "0°C"
    - date: "12/29(一)"
      weather: "大雪"
      low: "-9°C"
      high: "-2°C"
    - date: "12/30(二)"
      weather: "陰"
      low: "-5°C"
      high: "0°C"
    - date: "12/31(三)"
      weather: "雪"
      low: "-3°C"
      high: "3°C"
    - date: "1/1(四)"
      weather: "晴"
      low: "-4°C"
      high: "1°C"
    - date: "1/2(五)"
      weather: "陰"
      low: "8°C"
      high: "3°C"
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 42.93152
  longitude: 141.451489
elevation:
  top: 270
  bottom: 170
  verticalDrop: 100
courses:
  total: 5
  beginnerRatio: 50
  intermediateRatio: 50
  advancedRatio: 0
  courseInfoPage: "https://weathernews.jp/ski/spot/31416/"
  summary: "Dynasty Ski Resort共有 5 條雪道，難度比例約初級 50%、中級 50%、進階 0%，適合依雪況與同行者程度安排滑行路線。"
lifts:
  total: 3
access:
  fromSapporo:
    - label: "大眾交通工具（主要車站銜接）"
      steps:
        - "鐵路：JR千歳線北広島駅下車→バス・タクシーにて (30分)"
        - "鐵路：地下鉄東豊線福住駅または地下鉄東西線大谷地駅下車→バス・タクシーにて (30分)"
      estimatedTime: "約 30 分"
      difficulty: "medium"
  fromAirport:
    - label: "機場銜接"
      steps:
        - "飛機：新千歳空港より車で (45分)"
      estimatedTime: "約 45 分"
      difficulty: "medium"
  car:
    recommended: true
    snowTireRequired: true
    note: "自駕參考：自駕：道央道 北広島ICより国道36号を恵庭市方面へ、道道790号を仁別方面へ右折、距離6km、 (10分)。停車場：1000台 / 平日 無料 / 休日 無料。冬季山路需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 50%，可作為暖身、親子同行或初訪時的主要安排。"
  intermediate: "中級比例約 50%，適合已能穩定轉彎的滑雪者串接主要雪道。"
  advanced: "進階比例約 0%，建議視當日積雪與壓雪狀況選擇。"
  snowboard: "全面可 / 一部スノーボード専用パークあり；單板實際可滑區域仍以現場開放為準。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Dynasty Ski Resort Official Website"
    url: "http://www.dynasty-resort.jp/"
  - label: "Weathernews ダイナスティスキーリゾート"
    url: "https://weathernews.jp/ski/spot/31416/"
---

Dynasty Ski Resort位於北海道，共有 5 條雪道與 3 條纜車，是可放入北海道滑雪行程的地方型雪場。雪道比例約為初級 50%、中級 50%、進階 0%，建議出發前再確認官方公告、當日雪況與營業範圍。
