---
id: washigatake
name:
  zhTw: 鷲ヶ岳
  ja: 鷲ヶ岳スキー場
  en: Washigatake Ski Resort
region: chubu
prefecture: 岐阜縣
skiArea: takasu-mountains
tags:
- family_friendly
- night_skiing
- no_car_accessible
- snowboard_friendly
- onsen
links:
  official: https://ski.washigatake.jp/
  instagram: "https://www.instagram.com/washigatake5105/"
  xTwitter: "https://twitter.com/washigatake"
  trailMapPage: https://ski.washigatake.jp/gelande/
  ticket: https://www.takasumountains.com/lift/
  access: https://ski.washigatake.jp/access/
  snowReport: https://ski.washigatake.jp/condition/
  liftStatus: https://ski.washigatake.jp/condition/
  googleMaps: https://www.google.com/maps/search/?api=1&query=Washigatake%20Ski%20Resort
contact:
  address:
    zhTw: 日本岐阜縣郡上市高鷲町大鷲 3250
    ja: 〒501-5303 岐阜県郡上市高鷲町大鷲3250
    googleMaps: https://www.google.com/maps/search/?api=1&query=Washigatake%20Ski%20Resort
  phone: 0575-72-5105
season:
  label: 2025-2026
  operatingPeriod: 2025/11/21-2026/4/6，實際依積雪與官方公告調整
  hours: 全日 8:00-16:30
  nightSkiingHours: 週六限定夜滑，指定日 16:00-22:30
  source: https://www.takasumountains.com/ski/
trailMaps:
- label: 鷲ヶ岳 course map
  language: Japanese
  season: 2025-2026
  url: https://ski.washigatake.jp/gelande/
  sourceLabel: Washigatake Official
tickets:
  season: 2025-2026
  currency: JPY
  source: https://www.takasumountains.com/lift/
  note: TAKASU MOUNTAINS 6 雪場共通券摘要；全山共通券不含鷲ヶ岳夜滑。
  plans:
  - name: 6 雪場共通 1 日券
    audience: 成人、兒童、長者、中高生
    price: 成人 ¥6,500、兒童 ¥3,800、長者 ¥5,500、中高生 ¥4,500
snowWeather:
  title: 鷲ヶ岳雪況天氣
  provider: Weathernews
  url: https://weathernews.jp/ski/spot/35205/
  snowDepth: 0 cm
  updatedAt: 2026/4/30 09:00:00
  forecast: []
  note: 雪況顯示沿用建站期間的靜態佔位資料；出發前請以官方即時雪況為準。
location:
  latitude: 35.920
  longitude: 136.872
elevation:
  verticalDrop: 410
courses:
  total: 13
  beginnerRatio: 30
  intermediateRatio: 40
  advancedRatio: 30
  courseInfoPage: https://ski.washigatake.jp/gelande/
  summary: 鷲ヶ岳官方列出 13 條 course，最長 3,500m，特色是高鷲 IC 附近的便利交通、寬大名物坡面、park 與週六指定日夜滑。
  details:
  - name: ノースウェイコース / North Way Course
    difficulty: beginner
    length: 1,000m
    maxSlope: 20°
    averageSlope: 13°
    note: 從第 3 Quad 山頂進入的林間 course，中段之後以較狹窄緩坡為主。
  - name: イーグルコース / Eagle Course
    difficulty: beginner
    length: 700m
    maxSlope: 20°
    averageSlope: 13°
    note: 從第 2 Quad 山頂繞往初級路線，經 Panorama Course 可形成 2,500m long run。
  - name: パノラマコース / Panorama Course
    difficulty: beginner
    length: 1,800m
    maxSlope: 25°
    averageSlope: 15°
    note: 雖為初級 course，但有多個分岔，官方稱中上級者也常使用；中段寬坡開放感強。
  - name: マジカルコース / Magical Course
    difficulty: intermediate
    length: 400m
    maxSlope: 28°
    averageSlope: 20°
    note: 從第 2 Quad 下車處往第 3 Quad 的 course，官方說明斜度較強且常有饅頭。
  - name: アタックコース / Attack Course
    difficulty: intermediate
    length: 1,100m
    maxSlope: 30°
    averageSlope: 15°
    note: 夾在森林間的天然雪 course，官方說明常形成饅頭。
  - name: 林間コース / Rinkan Course
    difficulty: intermediate
    length: 1,100m
    maxSlope: 30°
    averageSlope: 20°
    note: 較狹窄且多為不整地的林間 course，進入前需要有心理準備。
  - name: ラビットコース / Rabbit Course
    difficulty: intermediate
    length: 1,300m
    maxSlope: 28°
    averageSlope: 15°
    note: 起點坡度稍強，後段轉緩並回到 center house，後段適合初學者練習。
  - name: ホワイトコース / White Course
    difficulty: intermediate
    length: 300m
    maxSlope: 25°
    averageSlope: 23°
    note: 官方 2025-2026 頁面標示此 course 更新中。
  - name: ブナ平コース / Buna-Daira Course
    difficulty: intermediate
    length: 1,600m
    maxSlope: 30°
    averageSlope: 17°
    note: 主要 downhill course 之一，在斜度變化中穿過林間部。
  - name: パラダイスコース / Paradise Course
    difficulty: advanced
    length: 1,000m
    maxSlope: 33°
    averageSlope: 20°
    note: 從山頂滑下的上級 course，起點為急斜面，之後接較舒服的緩斜面。
  - name: ノースバーンコース / North Barn Course
    difficulty: advanced
    length: 1,100m
    maxSlope: 30°
    averageSlope: 19°
    note: 第 1 Quad 沿線主 course，中段與起點都有急斜面。
  - name: パラレルコース / Parallel Course
    difficulty: advanced
    length: 1,300m
    maxSlope: 30°
    averageSlope: 17°
    note: 官方稱可體驗寬達 400m 的名物大坡面，適合上級者練習。
  - name: 尾根コース / Ridge Course
    difficulty: advanced
    length: 1,600m
    maxSlope: 35°
    averageSlope: 20°
    note: 從 Buna-Daira 分出的長距離路線，終段有 35° 壁面。
lifts:
  total: 4
access:
  fromNagoya:
    - label: 名古屋出發高速巴士與接駁
      steps:
        - 從名鐵巴士中心搭高速巴士到「高速ひるがの高原」。
        - 轉乘 Takasu Snow Shuttle Bus 鷲ヶ岳 LINE 前往鷲ヶ岳。
      estimatedTime: 約 2.5-3 小時
      difficulty: medium
      note: 也可利用名古屋出發的預約制 tour bus；免費接駁有季節與班次限制，需確認上下車站與回程班次。
  car:
    recommended: true
    snowTireRequired: true
    note: 東海北陸自動車道高鷲 IC 起約 3km；冬季需雪胎或防滑鏈，並留意高鷲周邊道路狀況。
terrainSummary:
  beginner: North Way、Eagle 與 Panorama 提供從山頂或中段迂迴的初級路線。
  intermediate: Rabbit、Buna-Daira、Attack 與 Rinkan 有林間、天然雪與坡度變化。
  advanced: Paradise、North Barn、Parallel、Ridge 是主要高難度線。
  snowboard: park 與寬大坡面適合 snowboarder；夜滑需另看指定日與夜滑券。
  powder: 林間與天然雪 course 會受降雪與開放狀態影響。
sources:
- label: Washigatake Course Guide
  url: https://ski.washigatake.jp/gelande/
- label: TAKASU MOUNTAINS Ski Area
  url: https://www.takasumountains.com/ski/
- label: TAKASU MOUNTAINS Lift Fee
  url: https://www.takasumountains.com/lift/
- label: Washigatake Access
  url: https://ski.washigatake.jp/access/
---

鷲ヶ岳位於岐阜縣的高鷲 Mountains，適合喜歡長線巡航的人，主要滑行線可拉到約 3,500 公尺。放進高鷲旅程時，能和周邊雪場形成不同節奏；適合想把滑雪後的泡湯、在地停留與雪國慢節奏放進同一天的人。也適合把雪地遊玩、練習與休息時間安排得更從容。
