---
id: "urabandai"
name:
  zhTw: "裏磐梯"
  ja: "裏磐梯スキー場"
  en: "Urabandai Ski Area"
region: "tohoku"
prefecture: "福島縣"
tags:
  - "beginner_friendly"
  - "family_friendly"
  - "no_car_accessible"
  - "onsen"
  - "snowboard_friendly"
visibility:
  status: "published"
  note: "福島縣雪場清單補新增的靜態初版；以官方資訊與公開雪場資料整理。"
links:
  official: "https://www.urabandai-ski.jp/"
  weather: "https://weathernews.jp/ski/spot/33626/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=37.632043,140.067616"
  access: "https://www.urabandai-ski.jp/"
contact:
  address:
    zhTw: "日本福島縣耶麻郡北鹽原村大字檜原字大府平山1170"
    ja: "〒969-2701 福島県耶麻郡北塩原村大字檜原字大府平山1170"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=37.632043,140.067616"
  phone: "0241-32-2177"
season:
  label: "2025-2026"
  operatingPeriod: "2025/12/19-2026/03/22"
  hours: "平日：8:30〜16:00(週一與週五) / 假日：8:30〜16:00(土日、國定假日、年末年始) / 夜滑：無"
  note: "營業日期與時間會依積雪、天候與活動調整；出發前請確認官方公告。"
  source: "https://weathernews.jp/ski/spot/33626/"
snowWeather:
  title: "裏磐梯 積雪與天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/33626/"
  snowDepth: 0 cm
  updatedAt: "2026/4/30 09:00:00"
  forecast: []
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 37.632043
  longitude: 140.067616
elevation:
  top: 1200
  bottom: 900
  verticalDrop: 300
courses:
  total: 3
  beginnerRatio: 40
  intermediateRatio: 30
  advancedRatio: 30
  courseInfoPage: "https://weathernews.jp/ski/spot/33626/"
  summary: "裏磐梯共有 3 條雪道，難度比例約初級 40%、中級 30%、進階 30%，適合依雪況與同行者程度安排滑行路線。"
  details:
    - name: "初級路線"
      difficulty: beginner
      note: "目前僅能確認此雪場包含初級滑行比例；正式雪道名稱、長度與坡度待後續以官方雪道圖補齊。"
    - name: "中級路線"
      difficulty: intermediate
      note: "目前僅能確認此雪場包含中級滑行比例；正式雪道名稱、長度與坡度待後續以官方雪道圖補齊。"
    - name: "上級路線"
      difficulty: advanced
      note: "目前僅能確認此雪場包含上級滑行比例；正式雪道名稱、長度與坡度待後續以官方雪道圖補齊。"
lifts:
  total: 3
access:
  fromTokyo:
    - label: "大眾交通工具（主要車站銜接）"
      steps:
        - "鐵路：東京駅(東北新幹線)-郡山駅(磐越西線快速)-猪苗代駅-路線バス (160分)"
      estimatedTime: "約 160 分"
      difficulty: "medium"
  car:
    recommended: true
    snowTireRequired: true
    note: "自駕參考：自駕：東京(東北自動車道)-郡山JCT(磐越自動車道)-猪苗代磐梯高原IC-国道115号-国道459号 (205分)。停車場：200台 / 平日 免費 / 假日 免費。冬季山路需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 40%，可作為暖身、親子同行或初訪時的主要安排。"
  intermediate: "中級比例約 30%，適合已能穩定轉彎的滑雪者串接主要雪道。"
  advanced: "進階比例約 30%，建議視當日積雪與壓雪狀況選擇。"
  snowboard: "全面可；單板實際可滑區域仍以現場開放為準。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Urabandai Ski Area Official Website"
    url: "https://www.urabandai-ski.jp/"
  - label: "Weathernews 裏磐梯スキー場"
    url: "https://weathernews.jp/ski/spot/33626/"
---

裏磐梯位於福島縣，重點不在刺激挑戰，而是讓同行者用舒服節奏熟悉雪地活動。行程可以安排得單純一點，把時間留給練習、休息與看雪景；適合把基礎練習、雪地遊玩與休息時間安排得更從容。停留時間可依練習、休息與雪景體驗彈性安排，不必把一天排得太緊。
