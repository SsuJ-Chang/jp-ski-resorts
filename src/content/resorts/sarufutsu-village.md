---
id: "sarufutsu-village"
name:
  zhTw: "猿拂村營"
  ja: "猿払村営スキー場"
  en: "Sarufutsu Village Ski Area"
region: "hokkaido"
prefecture: "北海道"
tags:
  - "beginner_friendly"
  - "night_skiing"
  - "no_car_accessible"
  - "onsen"
  - "snowboard_friendly"
visibility:
  status: "published"
  note: "北海道雪場清單補新增的靜態初版；以官方資訊與公開雪場資料整理。"
links:
  official: "https://weathernews.jp/ski/spot/31104/"
  weather: "https://weathernews.jp/ski/spot/31104/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=45.3348,142.1123"
  access: "https://weathernews.jp/ski/spot/31104/"
contact:
  address:
    zhTw: "日本北海道宗谷郡猿払村鬼志別西町238番地、及び225番地"
    ja: "〒098-6232 北海道宗谷郡猿払村鬼志別西町238番地、及び225番地"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=45.3348,142.1123"
  phone: "01635-2-3540"
season:
  label: "2025-2026"
  operatingPeriod: "2025/12/下旬-2026/02/下旬"
  hours: "平日：週一至週四 13:00〜16:00、週五 13:00〜21:00 / 假日：週六 10:00〜21:00、週日 10:00〜16:00 / 夜滑：週五、週六 17:30〜21:00"
  note: "營業日期與時間會依積雪、天候與活動調整；出發前請確認官方公告。"
  source: "https://weathernews.jp/ski/spot/31104/"
snowWeather:
  title: "猿拂村營 積雪與天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/31104/"
  snowDepth: "200 cm"
  updatedAt: "2026/4/30 09:00:00"
  forecast:
    - date: "12/27(六)"
      weather: "雪"
      low: "-5°C"
      high: "4°C"
    - date: "12/28(週日)"
      weather: "大雪"
      low: "-8°C"
      high: "0°C"
    - date: "12/29(一)"
      weather: "大雪"
      low: "-9°C"
      high: "-2°C"
    - date: "12/30(二)"
      weather: "陰"
      low: "-5°C"
      high: "0°C"
    - date: "12/31(三)"
      weather: "雪"
      low: "-3°C"
      high: "3°C"
    - date: "1/1(四)"
      weather: "晴"
      low: "-4°C"
      high: "1°C"
    - date: "1/2(五)"
      weather: "陰"
      low: "8°C"
      high: "3°C"
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 45.3348
  longitude: 142.1123
elevation:
  top: 52
  bottom: 10
  verticalDrop: 42
courses:
  total: 1
  beginnerRatio: 100
  intermediateRatio: 0
  advancedRatio: 0
  courseInfoPage: "https://weathernews.jp/ski/spot/31104/"
  summary: "猿拂村營共有 1 條雪道，難度比例約初級 100%、中級 0%、進階 0%，適合依雪況與同行者程度安排滑行路線。"
lifts:
  total: 1
access:
  fromSapporo:
    - label: "大眾交通工具（主要車站銜接）"
      steps:
        - "鐵路：稚内市→宗谷バス浜頓別方面行き→鬼志別バスターミナル→徒歩 (20分)"
      estimatedTime: "約 20 分"
      difficulty: "medium"
  car:
    recommended: true
    snowTireRequired: true
    note: "停車場：20台 / 平日 免費 / 假日 免費。冬季山路需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 100%，可作為暖身、親子同行或初訪時的主要安排。"
  intermediate: "中級比例約 0%，適合已能穩定轉彎的滑雪者串接主要雪道。"
  advanced: "進階比例約 0%，建議視當日積雪與壓雪狀況選擇。"
  snowboard: "全面可；單板實際可滑區域仍以現場開放為準。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Weathernews 猿払村営スキー場"
    url: "https://weathernews.jp/ski/spot/31104/"
---

猿拂村營位於北海道，白天與夜晚的雪面感不同，想延長滑行時間時會更有彈性。比起匆忙踩點，更適合用半天到一天熟悉雪面、坡感與山景；適合把運動量和放鬆感平衡在同一趟雪國旅程裡的人。整體適合放慢節奏，從雪面觸感、山景與停留氛圍中感受差異。
