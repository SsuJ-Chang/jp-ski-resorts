---
id: "zao-liza-world"
name:
  zhTw: "藏王 Liza World"
  ja: "蔵王ライザワールド"
  en: "Zao Liza World"
region: "tohoku"
prefecture: "山形縣"
tags:
  - "family_friendly"
  - "no_car_accessible"
  - "onsen"
  - "snowboard_friendly"
visibility:
  status: "published"
  note: "山形縣雪場清單補新增的靜態初版；以官方資訊與公開雪場資料整理。"
links:
  official: "http://www.zaoliza.co.jp/wntr/"
  weather: "https://weathernews.jp/ski/spot/33504/"
  googleMaps: "https://www.google.com/maps/search/?api=1&query=38.123957,140.405488"
  access: "http://www.zaoliza.co.jp/wntr/"
contact:
  address:
    zhTw: "日本山形縣上山市藏王坊平高原"
    ja: "〒999-3113 山形県上山市蔵王坊平高原"
    googleMaps: "https://www.google.com/maps/search/?api=1&query=38.123957,140.405488"
  phone: "023-679-2311"
season:
  label: "2025-2026"
  operatingPeriod: "2025/12/19-2026/03/29"
  hours: "平日：8:30〜16:00 / 休日：8:30〜16:30(1/11〜3/23） / ナイター：なし"
  note: "營業日期與時間會依積雪、天候與活動調整；出發前請確認官方公告。"
  source: "https://weathernews.jp/ski/spot/33504/"
snowWeather:
  title: "藏王 Liza World 積雪與天氣"
  provider: "Weathernews"
  url: "https://weathernews.jp/ski/spot/33504/"
  snowDepth: "200 cm"
  updatedAt: "2026/4/30 09:00:00"
  forecast:
    - date: "12/27(六)"
      weather: "雪"
      low: "-5°C"
      high: "4°C"
    - date: "12/28(日)"
      weather: "大雪"
      low: "-8°C"
      high: "0°C"
    - date: "12/29(一)"
      weather: "大雪"
      low: "-9°C"
      high: "-2°C"
    - date: "12/30(二)"
      weather: "陰"
      low: "-5°C"
      high: "0°C"
    - date: "12/31(三)"
      weather: "雪"
      low: "-3°C"
      high: "3°C"
    - date: "1/1(四)"
      weather: "晴"
      low: "-4°C"
      high: "1°C"
    - date: "1/2(五)"
      weather: "陰"
      low: "8°C"
      high: "3°C"
  note: "雪況與預報為網站建置期間的靜態示意資料；實際積雪、天氣與營業狀態請以官方公告與當日資訊為準。"
location:
  latitude: 38.123957
  longitude: 140.405488
elevation:
  top: 1446
  bottom: 1060
  verticalDrop: 386
courses:
  total: 6
  beginnerRatio: 33
  intermediateRatio: 43
  advancedRatio: 24
  courseInfoPage: "https://weathernews.jp/ski/spot/33504/"
  summary: "藏王 Liza World共有 6 條雪道，難度比例約初級 33%、中級 43%、進階 24%，適合依雪況與同行者程度安排滑行路線。"
lifts:
  total: 3
access:
  fromTokyo:
    - label: "大眾交通工具（主要車站銜接）"
      steps:
        - "鐵路：山形新幹線かみのやま温泉駅前 観光案内所前より無料バス (40分)"
        - "鐵路：山形新幹線山形駅から (60分)"
      estimatedTime: "約 40 分"
      difficulty: "medium"
  fromAirport:
    - label: "機場銜接"
      steps:
        - "飛機：仙台空港 (120分)"
      estimatedTime: "約 120 分"
      difficulty: "medium"
  car:
    recommended: true
    snowTireRequired: true
    note: "自駕參考：自駕：仙台宮城IC→山形蔵王IC経由 (60分)；自駕：川口JCT→かみのやま温泉IC経由 (40分)。停車場：1300台 / 平日 無料 / 休日 1台1,000円(1/4~3/9の土日祝)。冬季山路需準備雪胎或雪鏈。"
terrainSummary:
  beginner: "初級比例約 33%，可作為暖身、親子同行或初訪時的主要安排。"
  intermediate: "中級比例約 43%，適合已能穩定轉彎的滑雪者串接主要雪道。"
  advanced: "進階比例約 24%，建議視當日積雪與壓雪狀況選擇。"
  snowboard: "全面可 / コース外の滑走禁止；單板實際可滑區域仍以現場開放為準。"
externalContent:
  blogs: []
  vlogs: []
sources:
  - label: "Zao Liza World Official Website"
    url: "http://www.zaoliza.co.jp/wntr/"
  - label: "Weathernews 蔵王ライザワールド"
    url: "https://weathernews.jp/ski/spot/33504/"
---

藏王 Liza World位於山形縣，共有 6 條雪道與 3 條纜車，是可放入東北滑雪行程的地方型雪場。雪道比例約為初級 33%、中級 43%、進階 24%，建議出發前再確認官方公告、當日雪況與營業範圍。
